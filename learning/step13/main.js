import { mapImage, playerImage, enemyImageStraight, enemyImageHoming } from "./assets.js";
import { initInput, input, consumeUpgradePressed } from "./input.js";
import { STATE, createGame, resetToStart, startGame, tickTimers } from "./game.js";
import { weapon, upgradeWeapon } from "./weapon.js";
import { createPlayer, updatePlayer } from "./player.js";
import { bullets, resetShootCooldown, tryShoot, updateBullets } from "./bullets.js";
import { ENEMY_TYPE, enemies, spawnEnemy, updateEnemies } from "./enemies.js";
...
window.addEventListener("keydown", (e) => {
  ...
  if (game.state === STATE.START) {
    startGame(game, player, canvas, bullets, enemies, weapon);
    resetShootCooldown();
    return;
  }
  if (game.state === STATE.GAMEOVER) {
    resetToStart(game, player, canvas, bullets, enemies, weapon);
    resetShootCooldown();
    return;
  }
});
...
function updatePlaying() {
  ...
  if (consumeUpgradePressed()) upgradeWeapon();
  tryShoot(player, spaceHeld, weapon);
